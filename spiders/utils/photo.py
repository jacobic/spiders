...
def filter_conversion(names, directory):

    areas = []
    for n in names[:2]:
        photo_filter = np.loadtxt(os.path.join(directory, n))
        areas.append(auc(photo_filter[0], photo_filter[1]))

    return -2.51 * np.log(areas[0] / areas[1])
